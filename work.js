(function(){
  ...
  const PROJECTS = [
    ...
    },
    {
      ...
      blurb: "A complete website for a South Kingstown School Committee campaign, built around clear priorities, candidate information, and a responsive experience for voters.",
  ...
  function makeFlowScene(pal){
    ...
    function step(ctx,w,h,t){
      ...
      ctx.globalAlpha = 0.5;
      particles.forEach(p=>{
        const a = Math.sin(p.x*0.02+t*0.6)+Math.cos(p.y*0.024-t*0.5);
  ...
  function makeOrbitScene(pal){
    ...
    function init(w,h){
      ...
      for(let i=0;i<9;i++){
        nodes.push({
          r: 18 + Math.random()*Math.min(w,h)*0.42,
    ...
    function step(ctx,w,h,t){
      ...
      const pts = nodes.map(n=>{
        const ang = n.offset + t*n.speed;
        return { x: cx+Math.cos(ang)*n.r, y: cy+Math.sin(ang)*n.r*0.62, size:n.size };
      });
      for(let i=0;i<pts.length;i++){
        for(let j=i+1;j<pts.length;j++){
          ...
          if(d<thresh){
            ctx.strokeStyle = pal.faint(1-d/thresh);
            ctx.lineWidth = 1;
  ...
  function makeBranchScene(pal){
    ...
    function drawBranch(ctx,x,y,angle,len,depth,growth){
      ...
      const g = Math.max(0, Math.min(1, (growth-depth*0.11)*3));
      if(g<=0) return;
      ...
      if(g>=1){
        drawBranch(ctx, ex,ey, angle-0.42, len*0.72, depth-1, growth);
        drawBranch(ctx, ex,ey, angle+0.5, len*0.68, depth-1, growth);
      }
    }
    function step(ctx,w,h,t){
      ctx.globalAlpha=1; ctx.fillStyle = pal.bg; ctx.fillRect(0,0,w,h);
      const growth = Math.sin(t*0.35+phase*Math.PI*2)*0.5+0.5;
      drawBranch(ctx, w*0.5, h*0.98, -Math.PI/2, Math.min(w,h)*0.5, 6, growth);
    }
    return { init, step };
  }
  ...
  function makePulseScene(pal){
    ...
    function step(ctx,w,h,t){
      ctx.globalAlpha=1; ctx.fillStyle = pal.bg; ctx.fillRect(0,0,w,h);
      const cx = w*(0.5+0.3*Math.sin(t*0.3));
      const cy = h*(0.5+0.3*Math.cos(t*0.24));
      const cols = Math.ceil(w/cell)+1, rows = Math.ceil(h/cell)+1;
      for(let r=0;r<rows;r++){
        for(let c=0;c<cols;c++){
          const x=c*cell, y=r*cell;
          const d = Math.hypot(x-cx,y-cy);
          const wave = Math.max(0, Math.sin(d*0.05-t*2))*0.5+0.06;
          ctx.fillStyle = pal.faint(wave);
          ctx.beginPath(); ctx.arc(x,y,cell*0.32*(0.4+wave),0,Math.PI*2); ctx.fill();
        }
      }
    }
    return { init, step };
  ...
  PROJECTS.forEach((p, i)=>{
    ...
    tab.type = 'button';
    tab.className = 'pw-file-tab';
    tab.id = 'pwTab'+i;
    tab.setAttribute('role','tab');
    tab.setAttribute('aria-selected', i===0 ? 'true' : 'false');
    tab.setAttribute('aria-controls', 'pwPanel'+i);
    ...
    panel.className = 'pw-tabpanel' + (i===0 ? ' active' : '');
    panel.id = 'pwPanel'+i;
    panel.setAttribute('role','tabpanel');
    panel.setAttribute('aria-labelledby', 'pwTab'+i);
    if(i!==0) panel.hidden = true;

    const cmdline = document.createElement('div');
    cmdline.className = 'pw-panel-cmdline';
    const banner = document.createElement('div');
    banner.className = 'pw-panel-banner';
    ...
    content.innerHTML = `
      ...
      ${p.link && p.link !== '#'
        ? `<a class="pw-panel-link" href="${p.link}" target="_blank" rel="noopener noreferrer">view project &#8599;</a>`
        : `<span class="pw-panel-status">case study coming soon</span>`}
    `;
    ...
    const scene = (SCENE_FACTORIES[p.scene] || makeFlowScene)(pal);
    ...
    function resize(){
      ...
      if(reduceMotion){ scene.step(ctx, rect.width, rect.height, 2); }
    }
    function loop(now){
      if(!running) return;
      const rect = banner.getBoundingClientRect();
      scene.step(ctx, rect.width, rect.height, (now-tStart)/1000);
      requestAnimationFrame(loop);
    }
    new ResizeObserver(resize).observe(banner);
    resize();

    panels.push({ el:panel, cmdline, banner, content, titleEl:content.querySelector('.pw-panel-title'), title:p.title, file:p.file,
      start(){ if(running || reduceMotion) return; running = true; tStart = performance.now(); requestAnimationFrame(loop); },
      stop(){ running = false; }
    });
  });
  ...
  function typeCommand(el, text, speed){
    ...
    return new Promise(resolve=>{
      ...
      function step(){
        ...
        cursor.className = 'pw-cmd-cursor';
  ...
  function selectProject(i, moveFocus){
    ...
    if(currentIndex >= 0){
      panels[currentIndex].stop();
      panels[currentIndex].el.classList.remove('active');
      panels[currentIndex].el.hidden = true;
      panels[currentIndex].banner.classList.remove('show');
      panels[currentIndex].content.classList.remove('show');
    ...
    typeCommand(panel.cmdline, '$ cat '+panel.file).then(()=>{
      if(currentIndex !== i) return;
      panel.banner.classList.add('show');
      panel.content.classList.add('show');
      scrambleInto(panel.titleEl, panel.title, 550);
      if(sectionVisible) panel.start();
    });
  }
  ...
  sidebar.addEventListener('keydown', (e)=>{
    ...
    if(e.key==='ArrowDown'||e.key==='ArrowRight') next = (currentIndex+1) % tabs.length;
    else if(e.key==='ArrowUp'||e.key==='ArrowLeft') next = (currentIndex-1+tabs.length) % tabs.length;
  ...
  const headerIO = new IntersectionObserver((entries)=>{
    entries.forEach(entry=>{
      if(entry.isIntersecting){
        ...
        scrambleInto(document.getElementById('pwEyebrowText'), WORK_CONFIG.eyebrow, 450);
        scrambleInto(document.getElementById('pwHeadingText'), WORK_CONFIG.heading, 700);
  ...
  const visIO = new IntersectionObserver((entries)=>{
    entries.forEach(entry=>{
      ...
      if(sectionVisible && currentIndex >= 0){ panels[currentIndex].start(); }
      else if(!sectionVisible && currentIndex >= 0){ panels[currentIndex].stop(); }
    });
  }, { threshold:0.05 });
